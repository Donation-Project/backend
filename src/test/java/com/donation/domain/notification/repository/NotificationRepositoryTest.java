...
public class NotificationRepositoryTest extends RepositoryTest {
    ...
    void 회원아이디를_통해_확인하지않은_알림이_알림이_있는지_확인한다(){
        //given
        Long memberId = 0L;
        Notification notification = newPostToNotification(memberId, 1L);
        notificationRepository.save(notification);

        //when & then
        Assertions.assertThat(notificationRepository.existsByMemberIdAndConformIsFalse(memberId)).isTrue();
    }
    ...
    void 확인하지않은_알림을_모두_확인_상태로_변경한다(){
        ...
        List<Long> ids = notificationRepository.saveAll(LongStream.range(0, 5)
                        .mapToObj(i -> newPostToNotification(memberId, i))
        ...
        //then
        Assertions.assertThat(notificationRepository.existsByMemberIdAndConformIsFalse(memberId)).isFalse();
    }
    ...
    void 확인하지않은_알람을_조회한다(){
        ...
        //when
        List<Notification> actual = notificationRepository.findAllByMemberIdAndConformIsFalseOrderByIdDesc(memberId);
    ...
    void 회원의_알람을_모두_조회한다(){
        ...
        //when
        List<Notification> actual = notificationRepository.findAllByMemberIdOrderByIdDesc(memberId);
